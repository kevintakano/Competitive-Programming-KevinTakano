...
struct Job
{
    int day, inithour, initmin, endhour, endmin, profit;
};
...
int latestNonConflict(vector<Job> arr, int i)
{
    for (int j=i-1; j>=0; j--)
    {
        ...
        if(arr[j].endhour < arr[i].inithour && arr[j].day == arr[i].day )
        {
            return j;
        }   
        if(arr[j].endhour == arr[i].inithour && arr[j].endmin <= arr[i].initmin  &&  arr[j].day == arr[i].day )
        {
            return j;
        }
    }
...
void findMaxProfit(int n)
{
    ...
    Job aux; aux.inithour = 0, aux.endhour = 0, aux.profit = 0, aux.initmin = 0, aux.endmin = 0;
...
int main()
{
    ...
    while(cin >> N && N)
    {
        ...
        for0(i,N)
        {
            ...
            cin >> aux.inithour; cin >> c; cin >> aux.initmin;

            cin >> aux.endhour; cin >> c; cin >> aux.endmin; 
